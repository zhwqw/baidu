...
## 选择器  
### CSS子类选择器  
如果你不希望选择任意的后代元素，而是希望缩小范围，只选择某个元素的子元素，可以使用子类选择器。  
例如:h1>strong{color:blue;}选择h1下的第一个/子元素strong  
### 相邻兄弟选择器  
相邻兄弟选择器是指:选择紧接在另一元素后的元素，且二者有相同的父元素。  
如果要增加紧接在h1元素后出现的段落的上边距:h1+p{margin-top:50px;}  
